...
Nginx Proxy Manager erzeugt seine Host-Blöcke standardmäßig mit `proxy_set_header Host $host;` und `proxy_set_header X-Forwarded-Proto $scheme;` - beide Voraussetzungen sind damit ohne manuellen Eingriff erfüllt. Wer die generierte Konfiguration über *Advanced* → *Custom Nginx Configuration* erweitert, darf diese beiden Zeilen nicht entfernen oder überschreiben.
